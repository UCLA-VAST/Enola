...
class CodeGen():
    ...
    def __init__(
        ...
        with open(self.code_full_file, 'w') as f:
            json.dump(program.emit_full(), f, indent = 2)
        with open(self.code_full_file.replace('_code_full', '_code'), 'w') as f:
            json.dump(program.emit(), f, indent = 2)
...
class Animator():
    ...
    def update_init(self):
        ...
        self.row_plots = [self.ax.axhline(0, self.X_LOW, self.X_HIGH, c=(
            1, 0, 0, 0), ls='--') for _ in range(self.r_high)]
...
if __name__ == "__main__":
    ...
    parser.add_argument('--arch_param', help='hardware parameters for DPQA', type=str, default = "hardware_spec/default.json")
    ...
    with open('./graphs.json', 'r') as f:
        graphs = json.load(f)
    if str(num_qubit) in graphs.keys() and trial in range(10):
        gate_list = graphs[str(num_qubit)][trial]
    else:
        # import networkx as nx
        # gate_list = list(nx.random_regular_graph(3, args.size, args.id).edges)
    ...
    Animator(
        ...
        edges=gate_list if not args.noGraph else [],
